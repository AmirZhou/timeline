...
export const SyncStatusBar: React.FC = () => {
  ...
  const isVeryRecent = syncStatus.lastSyncTime && (Date.now() - new Date(syncStatus.lastSyncTime).getTime()) < 30 * 1000; // 30 seconds
        ...
        <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-yellow-800">
              Data may be propagating - if changes don't appear, wait 20-30 seconds and refresh again.
            </span>
          </div>
        </div>
      )}
